...
- Release archives must retain the compiled `daemon/` tree and locked manifests beside the executable. Verify those entries with `tar -tf` or `unzip -l`. Windows bootstrap uses `scripts/install.ps1` from the extracted ZIP.
