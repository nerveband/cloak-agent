...
The current daemon baseline is `cloakbrowser` `^0.5.2`. It supports explicit stable/preview release channels, exact browser version pins, extensions, authenticated proxy fixes, and the current Chromium 150 line. After upgrading that package, rebuild the daemon and run the integration tests because the launch wrappers depend on CloakBrowser's Playwright-compatible context APIs.
